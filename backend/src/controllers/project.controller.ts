...
export const extractCharacters = async (req: Request, res: Response): Promise<void> => {
  try {
    const rawId = req.params.id;
    const projectId = Array.isArray(rawId) ? rawId[0] : rawId;
    ...
    const updatedProject = await extractCharactersForProject(projectId, userId, Number(version));
...
export const generatePortraits = async (req: Request, res: Response): Promise<void> => {
  try {
    const rawId = req.params.id;
    const projectId = Array.isArray(rawId) ? rawId[0] : rawId;
    const { version } = req.body;
    ...
    const updatedProject = await generatePortraitsForProject(projectId, userId, Number(version));

    res.status(200).json({
      message: 'Portraits generated successfully.',
      project: updatedProject
    });
  } catch (error: any) {
    if (error.message === 'OCC_CONFLICT') {
